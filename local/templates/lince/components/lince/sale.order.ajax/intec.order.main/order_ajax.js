	...
	BX.Sale.OrderAjaxComponent = {
		...
		editSection: function(section)
		{
			...
			if (section.id == this.basketBlockNode.id) {
                ...
                if (this.result.SHOW_AUTH) {
                    editButton && BX.bind(titleNode, 'click', BX.delegate(function () {
                        this.animateScrollTo(this.authBlockNode);
		...
		editAuthBlock: function()
		{
            ...
            /* CUSTOM START */
            var authRow = BX.create('DIV', {props: {className: 'row'}});
            var regRow = BX.create('DIV', {props: {className: 'row'}});
			...
			if (this.result.SHOW_AUTH)
			{
                ...
                authContent.appendChild(authRow);
                this.editAuthorizeForm(authRow);
                this.editSocialContent(authRow);
                this.getAuthReference(authContent);

                this.getErrorContainer(regContent);
                regContent.appendChild(regRow);
                this.editRegistrationForm(regRow);
		...
		editAuthorizeForm: function(authContent)
		{
            ...
            remember = BX.create('DIV', {
                ...
                children: [
                    BX.create('LABEL', {
                        props: {className: 'bx-soa-checkbox'},
                        children: [
                            BX.create('INPUT', {
                                props: {
                                    type: 'checkbox',
                            ...
                            BX.create('SPAN', {props: {className: 'bx-soa-part-selector'}}),
                            BX.create('SPAN', {props: {className: 'bx-soa-part-content'}, text: BX.message('STOF_REMEMBER')})
                        ]
                    })
                ]
            });
			...
			button = BX.create('DIV', {
				...
				children: [
                    ...
                    BX.create('INPUT', {
                        props: {
                            type: 'submit',
                            className: 'bx-soa-button bx-soa-button-colored bx-soa-button-md',
		...
		editRegistrationForm: function(authContent)
		{
			...
			{
				/* CUSTOM START */
				var radioNode = BX.create('DIV', {props: {className: 'bx-authform-formgroup-container'}});

				authFormNodes.push(radioNode);

				/* CUSTOM END */

                radioNode.appendChild(
					/* CUSTOM */
                    BX.create('LABEL', {
                        props: {className: 'bx-soa-radiobox', for: 'NEW_GENERATE_N'},
                        children: [
                            BX.create('INPUT', {
							...
							BX.create('SPAN', {props: {className: 'bx-soa-part-selector'}}),
							BX.create('SPAN', {props: {className: 'bx-soa-part-content'}, text: BX.message('STOF_MY_PASSWORD')})
                        ],
                        events: {
                ...
                radioNode.appendChild(BX.create('BR'));
                radioNode.appendChild(
					/* CUSTOM */
                    BX.create('LABEL', {
                        props: {className: 'bx-soa-radiobox', for: 'NEW_GENERATE_Y'},
                        children: [
                            BX.create('INPUT', {
                                attrs: {checked: this.authGenerateUser},
                                props: {
                            ...
                            BX.create('SPAN', {props: {className: 'bx-soa-part-selector'}}),
                            BX.create('SPAN', {props: {className: 'bx-soa-part-content'}, text: BX.message('STOF_SYS_PASSWORD')})
                        ],
                        events: {
                            change: BX.delegate(function(){
                                var generated = this.authBlockNode.querySelector('.generated');
				...
				BX.create('DIV', {
					...
					children: [
                        ...
                        BX.create('INPUT', {
                            props: {
                                type: 'submit',
                                className: 'bx-authform-buttons-register bx-soa-button bx-soa-button-colored bx-soa-button-lg',
						...
						/* CUSTOM */
                        BX.create('A', {
                            props: {className: 'bx-authform-buttons-authorize', href: ''},
			...
			if (this.result.AUTH.new_user_registration === 'Y')
			{
				nodes.push(BX.create('DIV', {
					...
					children: [
						...
						/* CUSTOM */
                        BX.create('A', {
                            props: {className: 'bx-soa-button bx-soa-button-colored bx-soa-button-md'},
		...
		editFadeRegionContent: function(node)
		{
			...
			var errorNode = this.regionHiddenBlockNode.querySelector('.alert.alert-danger');

			var	addedHtml = '', props = [], locationProperty,
			...
			else if (personTypesCount == 2)
			{
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						...
						/* CUSTOM */
                        node.appendChild(BX.create('DIV', {props: {className: 'radio-inline' + (currentType.CHECKED == 'Y' ? ' radio-inline-checked' : '')}, children: [label]}));
		...
		createPaySystemItem: function(item)
		{
			...
			if(item.PSA_LOGOTIP_SRC_ORIGINAL !== ''){
				img = BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-img-container'},
					children: [
						BX.create('IMG', {
							props: {
								src: item.PSA_LOGOTIP_SRC_ORIGINAL,
								alt: ''
							}
						})
					]
		...
		editFadePaySystemContent: function(node)
		{
			...
			var errorNode = this.paySystemHiddenBlockNode.querySelector('div.alert.alert-danger');
			var warningNode = this.paySystemHiddenBlockNode.querySelector('div.alert.alert-warning.alert-show');
			var addedHtml = '', logotype, imgSrc;
		...
		getDeliveryExtraServices: function(delivery)
		{
			...
			for (i in delivery.EXTRA_SERVICES)
			{
				...
				else
				{
					/* CUSTOM */
                    serviceNode = BX.create('DIV', {
                        props: {className: 'bx-soa-pp-field bx-soa-pp-field-checkbox'},
                        children: [
                            BX.create('LABEL', {
                            	props: {className: 'bx-soa-checkbox'},
                                html: currentService.editControl
								+ '<span class="bx-soa-part-selector"></span>'
								+ '<span class="bx-soa-part-content">'
                    ...
                    if (currentService.description && currentService.description.length)
                    	serviceNode.appendChild(BX.create('DIV', {
                    		props: {className: 'bx-soa-service-small'},
                            html: BX.util.htmlspecialchars(currentService.description)
                        }));
		...
		createDeliveryItem: function(item)
		{
			...
			/* CUSTOM */
			var img, description, parameters;
            ...
            parameters = BX.create('DIV', {
            	props: {className: 'bx-soa-pp-company-parameters'}
			});
			...
			if(item.hasOwnProperty('LOGOTIP_SRC_ORIGINAL') && item.LOGOTIP_SRC_ORIGINAL !== ''){
				img = BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-img-container'},
					children: [
						BX.create('IMG', {
							props: {
								src: item.LOGOTIP_SRC_ORIGINAL,
								alt: ''
							}
						})
					]
		...
		editFadeDeliveryContent: function(node)
		{
			...
			var errorNode = this.deliveryHiddenBlockNode.querySelector('div.alert.alert-danger');
			var warningNode = this.deliveryHiddenBlockNode.querySelector('div.alert.alert-warning.alert-show');

			var extraService, logotype, imgSrc, arNodes, i;
		...
		selectDelivery: function(event)
		{
			...
			var target = event.target || event.srcElement,
				/* CUSTOM */
				actionParameters = BX.findParent(target, {className: 'bx-soa-pp-company-parameters'}),
				/* ORIGINAL */
				actionSection =  BX.hasClass(target, 'bx-soa-pp-company') ? target : BX.findParent(target, {className: 'bx-soa-pp-company'}),
		...
		activatePickUp: function(deliveryName)
		{
			...
			this.pickUpBlockNode.querySelector('h2.bx-soa-section-title').innerHTML =
				BX.util.htmlspecialchars(deliveryName);
				...
				children: [
					...
					BX.create('DIV', {
						...
						children: [
							/* CUSTOM */
                            BX.create('A', {
                                props: {href: '', className: 'bx-soa-button bx-soa-button-square bx-soa-button-colored'},
		...
		selectStore: function(event)
		{
			...
			if (storeItem && storeInput)
			{
				...
				button = BX.findChild(selectedPickUp, {className: 'bx-soa-button'}, true);

				if (button)
					button.text = this.params.MESS_SELECT_PICKUP;

				button = BX.findChild(storeItem, {className: 'bx-soa-button'}, true);

				if (button)
					button.text = this.params.MESS_SELECTED_PICKUP;
		...
		editPropsBlock: function(active)
		{
			...
			// перемещение полей
			$('#bx-soa-delivery').find('.bx-soa-section-content').append($('#zipProperty').parent());
			$('#bx-soa-delivery').find('.bx-soa-section-content').append($('[data-property-id-row=14]'));
			$('#bx-soa-delivery').find('.bx-soa-section-content').append($('[data-property-id-row=18]'));
			$('#bx-soa-delivery').find('.bx-soa-section-content').append($('[data-property-id-row=19]'));
		},
		...
		insertYNProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				/* CUSTOM */
                propContainer = BX.create('DIV', {props: {className: 'soa-property-container'}});
                propLabel = BX.create('LABEL', {props: {className: 'bx-soa-checkbox'}});
                property.appendTo(propLabel);
                propLabel.appendChild(BX.create('SPAN', {props: {className: 'bx-soa-part-selector'}}));
		...
		editPropsComment: function(propsNode)
		{
			...
			div = BX.create('DIV', {
				props: {className: 'form-group bx-soa-customer-field _max'},
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('A', {
								props: {
									href: 'javascript:void(0)',
									className: 'bx-soa-button bx-soa-button-block bx-soa-button-colored bx-soa-button-md btn-order-save'
								},
