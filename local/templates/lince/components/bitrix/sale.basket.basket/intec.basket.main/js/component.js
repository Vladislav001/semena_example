	...
	BX.Sale.BasketComponent = {
		...
		toggleFilter: function(event)
		{
			...
			for (var i = 0; i < entities.length; i++)
			{
				...
				{
					BX.addClass(entities[i], 'active intec-basket-scheme-border');
				}
				else if (BX.hasClass(entities[i], 'active'))
				{
					BX.removeClass(entities[i], 'active intec-basket-scheme-border');
				}
			}
		...
		bindImageEvents: function(node, data)
		{
			...
			var images = node.querySelectorAll('[data-product-property-role="image"]');
		...
		bindActionEvents: function(node, data)
		{
			...
			{
				entity = this.getEntities(node, 'basket-item-add-delayed');
                for (var i = 0, l = entity.length; i < l; i++)
                {
                    BX.bind(entity[i], 'click', BX.proxy(this.addDelayedAction, this));
                }
			}

            entity = this.getEntities(node, 'basket-item-remove-delayed');
            for (var i = 0, l = entity.length; i < l; i++)
            {
                BX.bind(entity[i], 'click', BX.proxy(this.removeDelayedAction, this));
            }
